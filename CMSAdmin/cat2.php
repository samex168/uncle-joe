<?php 
	...
	$title = "<a href='cat1.php'>".checkName("cat1", "title_tc", $cat1)."</a>";
	if(isset($_GET["id"]))
		$id = intval($_GET["id"]);
	...
	$result = mysql_query("select * from ".$tbl." where cat1= $cat1 order by seq limit $offset,$pagingSize");
	$totalRecord = mysql_num_rows(mysql_query("select * from ".$tbl." where cat1= $cat1 order by seq"));
...
<div class="col_content clearfix">
	<div class="col_main">
  	  <div class="content">
  	  	<span class="bdtxt c_txt13purple" style="float:right;margin-bottom:20px;"><?=$title?> - Cat II</span>
  		<input type="button" name="back" value="Back" onClick="window.location='cat1.php'" />
  		<input type="button" name="add" value="Add Cat II" onClick="window.location='<?=$module?>Add.php?cat1=<?=$cat1?>'" >
  		<div style="margin-top:10px;"><span id="paging"></span>
		<script>showPaging(<?=$totalPage?>, <?=$page?>, '<?=$module?>.php','&cat1=<?=$cat1?>');</script>
	    ...
	    <?php if($totalRecord > 0 ) { ?>
	  		<table border="0" cellpadding="2" cellspacing="1" width="100%" align="center" bgcolor="#6ed5f6" class="bdtxt">
				<tr>
					<td width="6%" align="center">Cat I</td>
					<td width="6%" align="center">Cat II</td>
			...
			<?php while($row = mysql_fetch_array($result)){ ?>
				<tr bgcolor="#FFFFFF" onMouseOver="bgColor='#CCFFFF'" onMouseOut="bgColor='#FFFFFF'" >
					...
					<td align="center" >
					 <input type="button" value="Edit" name="B222"onClick="window.location='<?=$module?>Edit.php?cat1=<?=$cat1?>&id=<?=$row["id"]?>'">
					<input type="button" value="Cat III" name="B22"onClick="window.location='cat3.php?cat1=<?=$cat1?>&cat2=<?=$row["id"]?>'">
					  </td>
				</tr>
			<?php } ?>
			</table>
